...
class class_centerness_regression_head(nn.Module):
    def __init__(self, fpn_output_channels=256, class_num=80,  output_channels=256,  grounp_norm=True, prior=0.01):
        ...
        self.regression_head = nn.Conv2d(output_channels, 4, kernel_size=3, stride=1, padding=1)
        self.center_ness_head = nn.Conv2d(output_channels, 1, kernel_size=3, stride=1, padding=1)
        self.classification_head = nn.Conv2d(output_channels, class_num, kernel_size=3, stride=1, padding=1)
    ...
    def _make_loc_head(self, head_output_channels, grounp_norm=True):
        layers = []
        for _ in range(4):
            layers.append(nn.Conv2d(256, head_output_channels, kernel_size=3, stride=1, padding=1))
            if grounp_norm:
                layers.append(nn.GroupNorm(32, head_output_channels))
            layers.append(nn.ReLU(True))
            layers.append(nn.Conv2d(head_output_channels, head_output_channels, kernel_size=3, stride=1, padding=1))
            if grounp_norm:
                layers.append(nn.GroupNorm(32, head_output_channels))
            layers.append(nn.ReLU(True))
        # in this head, in order to create centerness head, so, last conv don`t write in this function
        # layers.append(nn.Conv2d(head_output_channels, out_planes, kernel_size=3, stride=1, padding=1))
