...
class ClipBoxes(nn.Module):
    ...
    def forward(self, batch_imgs, ori_box):
        # print("ori_box is", ori_box)
        # batch_boxes = torch.tensor(item.cpu().detach().numpy() for item in ori_box)

        # batch_boxes = torch.tensor(ori_box).cuda()
        batch_boxes = ori_box.clamp_(min=0)
        h, w = batch_imgs.shape[2:]
        batch_boxes[..., [0, 2]] = batch_boxes[..., [0, 2]].clamp_(max=w - 1)
        batch_boxes[..., [1, 3]] = batch_boxes[..., [1, 3]].clamp_(max=h - 1)
        return batch_boxes

class fcosnet_post_process(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, inputs, batch_imgs, score_threshold, nms_iou_threshold, max_detection_boxes_num, strides, add_centerness):
        '''
        inputs  list [cls_logits,cnt_logits,reg_preds]
        cls_logits  list contains five [batch_size,class_num,h,w]
        cnt_logits  list contains five [batch_size,1,h,w]
    ...
    def _post_process(self, preds_topk):
        ...
        scores, classes, boxes = torch.stack(_cls_scores_post, dim=0), torch.stack(_cls_classes_post, dim=0), torch.stack(_boxes_post, dim=0)
        # return _cls_scores_post, _cls_classes_post, _boxes_post
        clip_boxes = self.clipbox(self.batch_imgs, _boxes_post[0])
        return _cls_scores_post[0], _cls_classes_post[0], clip_boxes
    ...
    def clipbox(self, batch_imgs, ori_box):
        batch_boxes = ori_box.clamp_(min=0)
        h, w = batch_imgs.shape[2:]
        batch_boxes[..., [0, 2]] = batch_boxes[..., [0, 2]].clamp_(max=w - 1)
        batch_boxes[..., [1, 3]] = batch_boxes[..., [1, 3]].clamp_(max=h - 1)
