...
from cfgs.fcosnet_quant_cfg import model_cfg as cfg
...
def main():
    ...
    for epoch_num in range(start_epoch+1, cfg["epochs"]):
        ...
        for iter_num, data in enumerate(dataloader_train):
            optimizer.zero_grad()
            cls_loss, cnt_loss, reg_loss, total_loss = model([data['img'].cuda().float(), data['annot']], is_training=True)
            cls_loss, cnt_loss, reg_loss, total_loss = cls_loss.mean(), cnt_loss.mean(), reg_loss.mean(), total_loss.mean()
            total_loss.backward()
            ...
            if iter_num%100 == 0:
                print('Runing lr: {} | Epoch: {} | Iteration: {} | cls_loss loss: {:1.5f} | cnt_loss loss: {:1.5f} | cnt_loss loss: {:1.5f} | Running loss: {:1.5f}' .format(
                    float(optimizer.param_groups[0]["lr"]),epoch_num, iter_num, float(cls_loss), float(cnt_loss), float(reg_loss), np.mean(loss_hist)) )
            del  cls_loss, cnt_loss, reg_loss, total_loss
        scheduler.step(np.mean(epoch_loss))
        print('Evaluating dataset')
        if epoch_num % 5 == 0:
            mAP = evaluate_datasets(dataset_val, model, cfg["dataset"]["type"], iou_threshold=cfg["iou_threshold"], score_threshold=cfg["score_threshold"], max_detections=cfg["max_detections"])
            if mAP != None:
                if mAP["mean_ap"] > best_ap:
                    best_ap = mAP["mean_ap"]
                    checkpoint = {"net":model.state_dict(), "optimizer":optimizer.state_dict(), "epoch":epoch_num}
                    torch.save(checkpoint, os.path.join(cfg["save_dir"], '{}_{}_model_epoch_{}_{}_map_{}_lr_{}.pt'.format(
