...
from cfgs.fcosnet_prune_cfg import model_cfg as cfg
...
def main():
    ...
    # if int(cfg["backbone"]["depth"]) in [18, 34, 50, 101, 152]:
    #     model  = prune_resnet.resnet50(num_classes=cfg["dataset"]["num_classes"], pretrained=True)
    ...
    for epoch_num in range(0, cfg["epochs"]):
        ...
        for iter_num, data in enumerate(dataloader_train):
            optimizer.zero_grad()
            cls_loss, cnt_loss, reg_loss, total_loss = prune_net([data['img'].cuda().float(), data['annot']], is_training=True)
